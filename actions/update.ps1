# OpenClaw Update Action - triggered by toast button or protocol handler
...
Write-Host "Updating $($cfg.npmPackage) in WSL ($($cfg.wslDistro))..."
wsl.exe -d $cfg.wslDistro -e bash -c "npm update -g $($cfg.npmPackage) 2>&1"

Write-Host "Restarting $($cfg.serviceName)..."
